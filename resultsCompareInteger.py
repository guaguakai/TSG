...
if __name__ == "__main__":
    ...
    obj_yn_int = np.zeros((Z,ZQ))
    obj_final_int = np.zeros((Z,ZQ))
    time_yn_int = np.zeros((Z,ZQ))
    time_final_int = np.zeros((Z,ZQ))
    ...
    for z in range(Z):
        ...
        for zq in range(ZQ):
            ...
            start_time_yn_int = time.time()
            
            obj_yn_int[z][zq], n, ns,ys,z_value,p,s,y = KStrategiesYNcombInteger(Q, W, K, R, M, P, resource2team, T, maxT, E, C, U_plus, U_minus, N_wk, shift, mr, minr, q, ar, phi, integer=0, OverConstr=False, OverConstr2=False)
            
            time_yn_int[z][zq] = time.time() - start_time_yn_int
    ...
    pickle.dump((obj_relax, obj_yn, obj_final, obj_yn_int, obj_final_int, time_relax, time_yn, time_final, time_yn_int, time_final_int), open("resultsCompareInteger_0125.data", "wb"))
    #obj_relax, obj_yn, obj_final, obj_yn_int, obj_final_int, time_relax, time_yn, time_final, time_yn_int, time_final_int = pickle.load(open("resultsCompareInteger_0125.data", "rb"))